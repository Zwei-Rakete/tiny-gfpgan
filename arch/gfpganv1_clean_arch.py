...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, x, style):
        ...
        # print("weight1",weight.shape)   #weight1 torch.Size([1, 512, 512, 3, 3])
        if self.demodulate:
            demod = torch.rsqrt(weight.pow(2).sum([2, 3, 4]) + self.eps)
            # print("demod",demod.shape)  #demod torch.Size([1, 512])
            weight = weight * demod.view(b, self.out_channels, 1, 1, 1)
            # print("weight2",weight.shape)   #weight2 torch.Size([1, 512, 512, 3, 3])

        weight = weight.view(b * self.out_channels, c, self.kernel_size, self.kernel_size)
        # print("weight3",weight.shape)   #weight3 torch.Size([512, 512, 3, 3])
        ...
        if self.sample_mode == 'upsample':
            ...
            x = self.interpolate2(x)
        b, c, h, w = x.shape
        # print("input",x.shape)  #input torch.Size([1, 512, 4, 4])
        x = x.view(1, b * c, h, w)  #x.view torch.Size([1, 512, 4, 4])
